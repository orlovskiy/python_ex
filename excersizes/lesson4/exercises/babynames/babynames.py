...
def extract_names(filename):
    if os.path.exists(filename):
        if os.path.isfile(filename):
            ...
            b = lambda x:[res[x].strip() for x in range(x + 1,x + 6)]
            names_dict = {res[a].strip():b(a) for a in range(1,len(res),7)}
        else:
...
def print_names(babynames):
    ...
    for i in years:
        print "{:-^22}".format(i)
    user_choice = str(input('Choose a year: '))
    if user_choice == 'q':
        sys.exit(0)
    elif user_choice in years:
        for key, value in sorted(babynames.items()):
            print key, value[years.index(user_choice)]
    else:
        print "{:*^42}".format('chosen date is incorrect')
