...
def match_ends(words):      # 2016.03.19_08:49:59 checked. prusanov
    count = 0
    for x in words:
...
def front_x(words):
    ...
    other_words=[]
    for x in words:
        if x[0] == "x":         # что будет, если x == '' ? (как PEP-8 рекомендует проверять префиксы?)
            x_words.append(x)
        else: 
...
def sort_last(lists):       # 2016.03.19_08:50:46 checked. prusanov
    def f(x):
        return x[-1]
    lists.sort(key = f)     # попробуйте решить задачу, используя lambda-функцию
    return lists
...
def remove_adjacent(nums):      # 2016.03.19_08:51:43 checked. prusanov
		prev = " "
		result = []
		for x in nums:
			if x != prev:
				...
				prev = x 
		return result
              	                  # в начале 2-го занятия немного про это задание рассказывал:
                                # заводим отдельную переменную для предыдущего значения на каждом шаге, 
                                # потом смело пробегаемся по списку и проверяем текущие значения,
                                # не забывая потом изменить предыдущего
